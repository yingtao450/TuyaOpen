 ...
 */
typedef void ble_gattc_err_fn(struct ble_gattc_proc *proc, int status, uint16_t att_handle);
 ...
 * to the appropriate active GATT procedure.
 */
typedef int ble_gattc_rx_adata_fn(struct ble_gattc_proc *proc, struct ble_att_read_type_adata *adata);

typedef int ble_gattc_rx_prep_fn(struct ble_gattc_proc *proc, int status, uint16_t handle, uint16_t offset,
                                 struct os_mbuf **om);

typedef int ble_gattc_rx_attr_fn(struct ble_gattc_proc *proc, int status, struct os_mbuf **om);
...
static os_membuf_t ble_gattc_proc_mem[OS_MEMPOOL_SIZE((TY_HS_BLE_GATT_MAX_PROCS), sizeof(struct ble_gattc_proc))];
...
static void ble_gattc_dbg_assert_proc_not_inserted(struct ble_gattc_proc *proc)
{
...
static void ble_gattc_log_disc_chr_uuid(struct ble_gattc_proc *proc)
{
    ...
    BLE_HS_LOG(INFO, "start_handle=%d end_handle=%d uuid=", proc->disc_chr_uuid.prev_handle + 1,
...
static void ble_gattc_log_read_uuid(uint16_t start_handle, uint16_t end_handle, const ble_uuid_t *uuid)
{
    ble_gattc_log_proc_init("read by uuid; ");
    BLE_HS_LOG(INFO, "start_handle=%d end_handle=%d uuid=", start_handle, end_handle);
...
static void ble_gattc_log_read_mult(const uint16_t *handles, uint8_t num_handles)
{
    int i;
...
static void ble_gattc_log_write_long(struct ble_gattc_proc *proc)
{
    ...
    BLE_HS_LOG(INFO, "att_handle=%d len=%d\n", proc->write_long.attr.handle, OS_MBUF_PKTLEN(proc->write_long.attr.om));
}
...
static void ble_gattc_log_write_reliable(struct ble_gattc_proc *proc)
{
    ...
    for (i = 0; i < proc->write_reliable.num_attrs; i++) {
        BLE_HS_LOG(INFO, "%s%d", i != 0 ? "," : "", proc->write_reliable.attrs[i].handle);
    }
    BLE_HS_LOG(INFO, "\n");
}
...
static const void *ble_gattc_rx_entry_find(uint8_t op, const void *rx_entries, int num_entries)
{
    struct gen_entry {
        uint8_t op;
        void (*cb)(void);
...
{
    proc->exp_os_ticks = tuya_ble_tick_count_get() + tuya_ble_time_ms_to_ticks32(BLE_GATTC_UNRESPONSIVE_TIMEOUT_MS);
}
...
static void ble_gattc_proc_set_resume_timer(struct ble_gattc_proc *proc)
{
    ...
    if (ble_gattc_resume_at == 0) {
        ble_gattc_resume_at = tuya_ble_tick_count_get() + tuya_ble_time_ms_to_ticks32((TY_HS_BLE_GATT_RESUME_RATE));
 ...
 */
static int ble_gattc_proc_matches_conn_op(struct ble_gattc_proc *proc, void *arg)
{
...
static int ble_gattc_proc_matches_expired(struct ble_gattc_proc *proc, void *arg)
{
...
static int ble_gattc_proc_matches_conn_rx_entry(struct ble_gattc_proc *proc, void *arg)
{
    struct ble_gattc_criteria_conn_rx_entry *criteria;

    criteria = arg;

    if (criteria->conn_handle != BLE_HS_CONN_HANDLE_NONE && criteria->conn_handle != proc->conn_handle) {

        return 0;
    }

    /* Entry matches; indicate corresponding rx entry. */
    criteria->matching_rx_entry = ble_gattc_rx_entry_find(proc->op, criteria->rx_entries, criteria->num_rx_entries);

    return (criteria->matching_rx_entry != NULL);
}

static void ble_gattc_extract(ble_gattc_match_fn *cb, void *arg, int max_procs, struct ble_gattc_proc_list *dst_list)
{
    struct ble_gattc_proc *proc;
...
static struct ble_gattc_proc *ble_gattc_extract_one(ble_gattc_match_fn *cb, void *arg)
{
...
static void ble_gattc_extract_by_conn_op(uint16_t conn_handle, uint8_t op, int max_procs,
                                         struct ble_gattc_proc_list *dst_list)
{
...
static struct ble_gattc_proc *ble_gattc_extract_first_by_conn_op(uint16_t conn_handle, uint8_t op)
{
    struct ble_gattc_proc_list dst_list;
...
static int ble_gattc_proc_matches_stalled(struct ble_gattc_proc *proc, void *unused)
{
...
static struct ble_gattc_proc *ble_gattc_extract_with_rx_entry(uint16_t conn_handle, const void *rx_entries,
                                                              int num_rx_entries, const void **out_rx_entry)
{
    struct ble_gattc_criteria_conn_rx_entry criteria;
    struct ble_gattc_proc *proc;
...
#define BLE_GATTC_RX_EXTRACT_RX_ENTRY(conn_handle, rx_entries, out_rx_entry)                                           \
    ble_gattc_extract_with_rx_entry((conn_handle), (rx_entries), sizeof(rx_entries) / sizeof(rx_entries)[0],           \
...
static int ble_gattc_mtu_cb(struct ble_gattc_proc *proc, int status, uint16_t att_handle, uint16_t mtu)
{
    ...
    if (proc->mtu.cb == NULL) {
        ...
        rc = proc->mtu.cb(proc->conn_handle, ble_gattc_error(status, att_handle), mtu, proc->mtu.cb_arg);
    }

    return rc;
}

static __attribute__((unused)) void ble_gattc_mtu_tmo(struct ble_gattc_proc *proc)
{
    BLE_HS_DBG_ASSERT(!ble_hs_locked_by_cur_task());
    ble_gattc_dbg_assert_proc_not_inserted(proc);
 ...
 */
static __attribute__((unused)) void ble_gattc_mtu_err(struct ble_gattc_proc *proc, int status, uint16_t att_handle)
{
    ble_gattc_dbg_assert_proc_not_inserted(proc);
 ...
 *                                  is no callback).
 */
static int ble_gattc_disc_all_svcs_cb(struct ble_gattc_proc *proc, uint16_t status, uint16_t att_handle,
                                      struct ble_gatt_svc *service)
{
    ...
    if (proc->disc_all_svcs.cb == NULL) {
        ...
        rc = proc->disc_all_svcs.cb(proc->conn_handle, ble_gattc_error(status, att_handle), service,
...
static __attribute__((unused)) void ble_gattc_disc_all_svcs_tmo(struct ble_gattc_proc *proc)
{
    BLE_HS_DBG_ASSERT(!ble_hs_locked_by_cur_task());
    ble_gattc_dbg_assert_proc_not_inserted(proc);
...
static int ble_gattc_disc_all_svcs_tx(struct ble_gattc_proc *proc)
{
    ...
    rc = ble_att_clt_tx_read_group_type(proc->conn_handle, proc->disc_all_svcs.prev_handle + 1, 0xffff, &uuid.u);
    if (rc != 0) {
        return rc;
    }
 ...
 */
static __attribute__((unused)) void ble_gattc_disc_all_svcs_err(struct ble_gattc_proc *proc, int status,
                                                                uint16_t att_handle)
{
    ble_gattc_dbg_assert_proc_not_inserted(proc);
 ...
 */
static int ble_gattc_disc_all_svcs_rx_adata(struct ble_gattc_proc *proc, struct ble_att_read_group_type_adata *adata)
{
    struct ble_gatt_svc service;
    int cbrc;
    int rc;
    ...
    case 2:
    case 16:
        rc = ble_uuid_init_from_att_buf(&service.uuid, adata->value, adata->value_len);
        if (rc != 0) {
            rc = BLE_HS_EBADDATA;
            goto done;
        }
 ...
 * fully processed.
 */
static int ble_gattc_disc_all_svcs_rx_complete(struct ble_gattc_proc *proc, int status)
{
    int rc;
 ...
 *                                  is no callback).
 */
static int ble_gattc_disc_svc_uuid_cb(struct ble_gattc_proc *proc, int status, uint16_t att_handle,
                                      struct ble_gatt_svc *service)
{
    int rc;
    ...
    if (proc->disc_svc_uuid.cb == NULL) {
        ...
        rc = proc->disc_svc_uuid.cb(proc->conn_handle, ble_gattc_error(status, att_handle), service,
...
static __attribute__((unused)) void ble_gattc_disc_svc_uuid_tmo(struct ble_gattc_proc *proc)
{
    BLE_HS_DBG_ASSERT(!ble_hs_locked_by_cur_task());
    ble_gattc_dbg_assert_proc_not_inserted(proc);
...
static int ble_gattc_disc_svc_uuid_tx(struct ble_gattc_proc *proc)
{
    ...
    rc = ble_att_clt_tx_find_type_value(proc->conn_handle, proc->disc_svc_uuid.prev_handle + 1, 0xffff,
 ...
 */
static __attribute__((unused)) void ble_gattc_disc_svc_uuid_err(struct ble_gattc_proc *proc, int status,
                                                                uint16_t att_handle)
{
    ble_gattc_dbg_assert_proc_not_inserted(proc);
 ...
 *                                  is no callback).
 */
static int ble_gattc_find_inc_svcs_cb(struct ble_gattc_proc *proc, int status, uint16_t att_handle,
                                      struct ble_gatt_svc *service)
{
    int rc;
    ...
    if (proc->find_inc_svcs.cb == NULL) {
        ...
        rc = proc->find_inc_svcs.cb(proc->conn_handle, ble_gattc_error(status, att_handle), service,
...
static __attribute__((unused)) void ble_gattc_find_inc_svcs_tmo(struct ble_gattc_proc *proc)
{
    BLE_HS_DBG_ASSERT(!ble_hs_locked_by_cur_task());
    ble_gattc_dbg_assert_proc_not_inserted(proc);
...
static int ble_gattc_find_inc_svcs_tx(struct ble_gattc_proc *proc)
{
    ...
    if (proc->find_inc_svcs.cur_start == 0) {
        ...
        rc = ble_att_clt_tx_read_type(proc->conn_handle, proc->find_inc_svcs.prev_handle + 1,
        ...
        rc = ble_att_clt_tx_read(proc->conn_handle, proc->find_inc_svcs.cur_start);
        if (rc != 0) {
            return rc;
        }
    }
 ...
 */
static __attribute__((unused)) void ble_gattc_find_inc_svcs_err(struct ble_gattc_proc *proc, int status,
                                                                uint16_t att_handle)
{
    ble_gattc_dbg_assert_proc_not_inserted(proc);

    if (proc->find_inc_svcs.cur_start == 0 && status == BLE_HS_ATT_ERR(BLE_ATT_ERR_ATTR_NOT_FOUND)) {
 ...
 * proc.
 */
static int ble_gattc_find_inc_svcs_rx_read_rsp(struct ble_gattc_proc *proc, int status, struct os_mbuf **om)
{
    struct ble_gatt_svc service;
    int rc;
 ...
 */
static int ble_gattc_find_inc_svcs_rx_adata(struct ble_gattc_proc *proc, struct ble_att_read_type_adata *adata)
{
    struct ble_gatt_svc service;
    int call_cb;
    int cbrc;
...
int ble_gattc_find_inc_svcs(uint16_t conn_handle, uint16_t start_handle, uint16_t end_handle, ble_gatt_disc_svc_fn *cb,
                            void *cb_arg)
{
 ...
 *                                  is no callback).
 */
static int ble_gattc_disc_all_chrs_cb(struct ble_gattc_proc *proc, int status, uint16_t att_handle,
                                      struct ble_gatt_chr *chr)
{
    ...
    if (proc->disc_all_chrs.cb == NULL) {
        ...
        rc = proc->disc_all_chrs.cb(proc->conn_handle, ble_gattc_error(status, att_handle), chr,
...
static __attribute__((unused)) void ble_gattc_disc_all_chrs_tmo(struct ble_gattc_proc *proc)
{
    BLE_HS_DBG_ASSERT(!ble_hs_locked_by_cur_task());
    ble_gattc_dbg_assert_proc_not_inserted(proc);
...
static int ble_gattc_disc_all_chrs_tx(struct ble_gattc_proc *proc)
{
    ...
    rc = ble_att_clt_tx_read_type(proc->conn_handle, proc->disc_all_chrs.prev_handle + 1,
 ...
 */
static __attribute__((unused)) void ble_gattc_disc_all_chrs_err(struct ble_gattc_proc *proc, int status,
                                                                uint16_t att_handle)
{
    ble_gattc_dbg_assert_proc_not_inserted(proc);
 ...
 */
static int ble_gattc_disc_all_chrs_rx_adata(struct ble_gattc_proc *proc, struct ble_att_read_type_adata *adata)
{
    struct ble_gatt_chr chr;
    int cbrc;
    int rc;
    ...
    case BLE_GATT_CHR_DECL_SZ_128:
        rc = ble_uuid_init_from_att_buf(&chr.uuid, adata->value + 3, adata->value_len - 3);
        if (rc != 0) {
            rc = BLE_HS_EBADDATA;
            goto done;
        }
...
int ble_gattc_disc_all_chrs(uint16_t conn_handle, uint16_t start_handle, uint16_t end_handle, ble_gatt_chr_fn *cb,
                            void *cb_arg)
{
 ...
 *                                  is no callback).
 */
static int ble_gattc_disc_chr_uuid_cb(struct ble_gattc_proc *proc, int status, uint16_t att_handle,
                                      struct ble_gatt_chr *chr)
{
    int rc;
    ...
    if (proc->disc_chr_uuid.cb == NULL) {
        ...
        rc = proc->disc_chr_uuid.cb(proc->conn_handle, ble_gattc_error(status, att_handle), chr,
...
static __attribute__((unused)) void ble_gattc_disc_chr_uuid_tmo(struct ble_gattc_proc *proc)
{
    BLE_HS_DBG_ASSERT(!ble_hs_locked_by_cur_task());
    ble_gattc_dbg_assert_proc_not_inserted(proc);
...
static int ble_gattc_disc_chr_uuid_tx(struct ble_gattc_proc *proc)
{
    ...
    rc = ble_att_clt_tx_read_type(proc->conn_handle, proc->disc_chr_uuid.prev_handle + 1,
 ...
 * discover-characteristic-by-uuid proc.
 */
static __attribute__((unused)) void ble_gattc_disc_chr_uuid_err(struct ble_gattc_proc *proc, int status,
                                                                uint16_t att_handle)
{
    ble_gattc_dbg_assert_proc_not_inserted(proc);
 ...
 */
static int ble_gattc_disc_chr_uuid_rx_adata(struct ble_gattc_proc *proc, struct ble_att_read_type_adata *adata)
{
    struct ble_gatt_chr chr;
    int cbrc;
    int rc;
...
int ble_gattc_disc_chrs_by_uuid(uint16_t conn_handle, uint16_t start_handle, uint16_t end_handle,
 ...
 *                                  is no callback).
 */
static int ble_gattc_disc_all_dscs_cb(struct ble_gattc_proc *proc, int status, uint16_t att_handle,
...
static __attribute__((unused)) void ble_gattc_disc_all_dscs_tmo(struct ble_gattc_proc *proc)
{
    BLE_HS_DBG_ASSERT(!ble_hs_locked_by_cur_task());
    ble_gattc_dbg_assert_proc_not_inserted(proc);
...
static int ble_gattc_disc_all_dscs_tx(struct ble_gattc_proc *proc)
{
    ...
    rc = ble_att_clt_tx_find_info(proc->conn_handle, proc->disc_all_dscs.prev_handle + 1,
                                  proc->disc_all_dscs.end_handle);
    if (rc != 0) {
        return rc;
    }
 ...
 */
static __attribute__((unused)) void ble_gattc_disc_all_dscs_err(struct ble_gattc_proc *proc, int status,
                                                                uint16_t att_handle)
{
    ble_gattc_dbg_assert_proc_not_inserted(proc);
...
int ble_gattc_disc_all_dscs(uint16_t conn_handle, uint16_t start_handle, uint16_t end_handle, ble_gatt_dsc_fn *cb,
                            void *cb_arg)
{
...
static int ble_gattc_read_cb(struct ble_gattc_proc *proc, int status, uint16_t att_handle, struct ble_gatt_attr *attr)
{
    ...
    if (proc->read.cb == NULL) {
        ...
        rc = proc->read.cb(proc->conn_handle, ble_gattc_error(status, att_handle), attr, proc->read.cb_arg);
    }

    return rc;
}

static __attribute__((unused)) void ble_gattc_read_tmo(struct ble_gattc_proc *proc)
{
    BLE_HS_DBG_ASSERT(!ble_hs_locked_by_cur_task());
    ble_gattc_dbg_assert_proc_not_inserted(proc);
 ...
 */
static __attribute__((unused)) void ble_gattc_read_err(struct ble_gattc_proc *proc, int status, uint16_t att_handle)
{
    ble_gattc_dbg_assert_proc_not_inserted(proc);
 ...
 *                                  is no callback).
 */
static int ble_gattc_read_uuid_cb(struct ble_gattc_proc *proc, int status, uint16_t att_handle,
                                  struct ble_gatt_attr *attr)
{
    int rc;
    ...
    if (proc->read_uuid.cb == NULL) {
        ...
        rc = proc->read_uuid.cb(proc->conn_handle, ble_gattc_error(status, att_handle), attr, proc->read_uuid.cb_arg);
    }

    return rc;
}

static __attribute__((unused)) void ble_gattc_read_uuid_tmo(struct ble_gattc_proc *proc)
{
    BLE_HS_DBG_ASSERT(!ble_hs_locked_by_cur_task());
    ble_gattc_dbg_assert_proc_not_inserted(proc);
 ...
 */
static __attribute__((unused)) void ble_gattc_read_uuid_err(struct ble_gattc_proc *proc, int status,
                                                            uint16_t att_handle)
{
    ble_gattc_dbg_assert_proc_not_inserted(proc);
 ...
 */
static int ble_gattc_read_uuid_rx_complete(struct ble_gattc_proc *proc, int status)
{
    ble_gattc_dbg_assert_proc_not_inserted(proc);
...
{
    return ble_att_clt_tx_read_type(proc->conn_handle, proc->read_uuid.start_handle, proc->read_uuid.end_handle,
                                    &proc->read_uuid.chr_uuid.u);
}

int ble_gattc_read_by_uuid(uint16_t conn_handle, uint16_t start_handle, uint16_t end_handle, const ble_uuid_t *uuid,
 ...
 *                                  is no callback).
 */
static int ble_gattc_read_long_cb(struct ble_gattc_proc *proc, int status, uint16_t att_handle,
                                  struct ble_gatt_attr *attr)
{
    int rc;
    ...
    if (proc->read_long.cb == NULL) {
        ...
        rc = proc->read_long.cb(proc->conn_handle, ble_gattc_error(status, att_handle), attr, proc->read_long.cb_arg);
    }

    return rc;
}

static __attribute__((unused)) void ble_gattc_read_long_tmo(struct ble_gattc_proc *proc)
{
    BLE_HS_DBG_ASSERT(!ble_hs_locked_by_cur_task());
    ble_gattc_dbg_assert_proc_not_inserted(proc);
...
static int ble_gattc_read_long_tx(struct ble_gattc_proc *proc)
{
    ...
    if (proc->read_long.offset == 0) {
        ...
        rc = ble_att_clt_tx_read_blob(proc->conn_handle, proc->read_long.handle, proc->read_long.offset);
        if (rc != 0) {
            return rc;
        }
    }
 ...
 */
static __attribute__((unused)) void ble_gattc_read_long_err(struct ble_gattc_proc *proc, int status,
                                                            uint16_t att_handle)
{
    ble_gattc_dbg_assert_proc_not_inserted(proc);
 ...
 */
static int ble_gattc_read_long_rx_read_rsp(struct ble_gattc_proc *proc, int status, struct os_mbuf **om)
{
    struct ble_gatt_attr attr;
    uint16_t data_len;
    uint16_t mtu;
 ...
 *                                  is no callback).
 */
static int ble_gattc_read_mult_cb(struct ble_gattc_proc *proc, int status, uint16_t att_handle, struct os_mbuf **om)
{
    struct ble_gatt_attr attr;
    int rc;
    ...
    if (proc->read_mult.cb == NULL) {
        ...
        rc = proc->read_mult.cb(proc->conn_handle, ble_gattc_error(status, att_handle), &attr, proc->read_mult.cb_arg);
    }
...
static __attribute__((unused)) void ble_gattc_read_mult_tmo(struct ble_gattc_proc *proc)
{
    BLE_HS_DBG_ASSERT(!ble_hs_locked_by_cur_task());
    ble_gattc_dbg_assert_proc_not_inserted(proc);
 ...
 */
static __attribute__((unused)) void ble_gattc_read_mult_err(struct ble_gattc_proc *proc, int status,
                                                            uint16_t att_handle)
{
    ble_gattc_dbg_assert_proc_not_inserted(proc);
...
static int ble_gattc_read_mult_tx(struct ble_gattc_proc *proc)
{
    ...
    rc = ble_att_clt_tx_read_mult(proc->conn_handle, proc->read_mult.handles, proc->read_mult.num_handles);
    if (rc != 0) {
        return rc;
    }
...
int ble_gattc_read_mult(uint16_t conn_handle, const uint16_t *handles, uint8_t num_handles, ble_gatt_attr_fn *cb,
                        void *cb_arg)
{
...
static int ble_gattc_write_cb(struct ble_gattc_proc *proc, int status, uint16_t att_handle)
{
    ...
    if (proc->write.cb == NULL) {
        ...
        rc = proc->write.cb(proc->conn_handle, ble_gattc_error(status, att_handle), &attr, proc->write.cb_arg);
    }

    return rc;
}

static __attribute__((unused)) void ble_gattc_write_tmo(struct ble_gattc_proc *proc)
{
    BLE_HS_DBG_ASSERT(!ble_hs_locked_by_cur_task());
    ble_gattc_dbg_assert_proc_not_inserted(proc);
 ...
 */
static __attribute__((unused)) void ble_gattc_write_err(struct ble_gattc_proc *proc, int status, uint16_t att_handle)
{
    ble_gattc_dbg_assert_proc_not_inserted(proc);
    ble_gattc_write_cb(proc, status, att_handle);
}

int ble_gattc_write(uint16_t conn_handle, uint16_t attr_handle, struct os_mbuf *txom, ble_gatt_attr_fn *cb,
                    void *cb_arg)
{
 ...
 *                                  is no callback).
 */
static int ble_gattc_write_long_cb(struct ble_gattc_proc *proc, int status, uint16_t att_handle)
{
    int rc;
    ...
    if (proc->write_long.cb == NULL) {
        ...
        rc = proc->write_long.cb(proc->conn_handle, ble_gattc_error(status, att_handle), &proc->write_long.attr,
...
static __attribute__((unused)) void ble_gattc_write_long_tmo(struct ble_gattc_proc *proc)
{
    BLE_HS_DBG_ASSERT(!ble_hs_locked_by_cur_task());
    ble_gattc_dbg_assert_proc_not_inserted(proc);
...
static int ble_gattc_write_long_tx(struct ble_gattc_proc *proc)
{
    ...
    write_len = MIN_CMP(max_sz, OS_MBUF_PKTLEN(proc->write_long.attr.om) - proc->write_long.attr.offset);

    if (write_len <= 0) {
        rc = ble_att_clt_tx_exec_write(proc->conn_handle, BLE_ATT_EXEC_WRITE_F_EXECUTE);
        goto done;
    }
    ...
    rc = os_mbuf_appendfrom(om, proc->write_long.attr.om, proc->write_long.attr.offset, proc->write_long.length);
    if (rc != 0) {
        rc = BLE_HS_ENOMEM;
        goto done;
    }

    rc = ble_att_clt_tx_prep_write(proc->conn_handle, proc->write_long.attr.handle, proc->write_long.attr.offset, om);
    om = NULL;
    if (rc != 0) {
        goto done;
    }
 ...
 * write-long-characteristic-value proc.
 */
static __attribute__((unused)) void ble_gattc_write_long_err(struct ble_gattc_proc *proc, int status,
                                                             uint16_t att_handle)
{
    ble_gattc_dbg_assert_proc_not_inserted(proc);
     ...
     */
    if (proc->write_long.attr.offset > 0 && proc->write_long.attr.offset < OS_MBUF_PKTLEN(proc->write_long.attr.om)) {
 ...
 */
static int ble_gattc_write_long_rx_prep(struct ble_gattc_proc *proc, int status, uint16_t handle, uint16_t offset,
                                        struct os_mbuf **rxom)
{
    struct os_mbuf *om;
    int rc;
    ...
    if (os_mbuf_cmpm(om, 0, proc->write_long.attr.om, offset, proc->write_long.length) != 0) {
...
int ble_gattc_write_long(uint16_t conn_handle, uint16_t attr_handle, uint16_t offset, struct os_mbuf *txom,
                         ble_gatt_attr_fn *cb, void *cb_arg)
{
#if !(TY_HS_BLE_GATT_WRITE_LONG)
    return BLE_HS_ENOTSUP;
...
static int ble_gattc_write_reliable_cb(struct ble_gattc_proc *proc, int status, uint16_t att_handle)
{
    ...
    if (proc->write_reliable.cb == NULL) {
        ...
        rc = proc->write_reliable.cb(proc->conn_handle, ble_gattc_error(status, att_handle), proc->write_reliable.attrs,
                                     proc->write_reliable.num_attrs, proc->write_reliable.cb_arg);
    }

    return rc;
}

static __attribute__((unused)) void ble_gattc_write_reliable_tmo(struct ble_gattc_proc *proc)
{
    BLE_HS_DBG_ASSERT(!ble_hs_locked_by_cur_task());
    ble_gattc_dbg_assert_proc_not_inserted(proc);
...
static int ble_gattc_write_reliable_tx(struct ble_gattc_proc *proc)
{
    ...
    proc->write_reliable.length = MIN_CMP(max_sz, OS_MBUF_PKTLEN(attr->om) - attr->offset);
    ...
    rc = os_mbuf_appendfrom(om, attr->om, attr->offset, proc->write_reliable.length);
    if (rc != 0) {
        rc = BLE_HS_ENOMEM;
        goto done;
    }

    rc = ble_att_clt_tx_prep_write(proc->conn_handle, attr->handle, attr->offset, om);
    om = NULL;
    if (rc != 0) {
        goto done;
    }
 ...
 */
static int ble_gattc_write_reliable_rx_prep(struct ble_gattc_proc *proc, int status, uint16_t handle, uint16_t offset,
                                            struct os_mbuf **rxom)
{
    struct ble_gatt_attr *attr;
    struct os_mbuf *om;
...
int ble_gattc_notify_custom(uint16_t conn_handle, uint16_t chr_val_handle, struct os_mbuf *txom)
{
    ...
    if (txom == NULL) {
        ...
        rc = ble_att_svr_read_handle(BLE_HS_CONN_HANDLE_NONE, chr_val_handle, 0, txom, NULL);
        if (rc != 0) {
 ...
 */
static __attribute__((unused)) void ble_gattc_indicate_err(struct ble_gattc_proc *proc, int status, uint16_t att_handle)
{
    int rc;
    ...
    if (status != BLE_HS_ENOTCONN) {
        rc = ble_gatts_rx_indicate_ack(proc->conn_handle, proc->indicate.chr_val_handle);
        if (rc != 0) {
            return;
        }
    }
    ...
    ble_gap_notify_tx_event(status, proc->conn_handle, proc->indicate.chr_val_handle, 1);

    /* Send the next indication if one is pending. */
    ble_gatts_send_next_indicate(proc->conn_handle);
}

static __attribute__((unused)) void ble_gattc_indicate_tmo(struct ble_gattc_proc *proc)
{
    BLE_HS_DBG_ASSERT(!ble_hs_locked_by_cur_task());
    ble_gattc_dbg_assert_proc_not_inserted(proc);

    ble_gap_notify_tx_event(BLE_HS_ETIMEOUT, proc->conn_handle, proc->indicate.chr_val_handle, 1);
}
...
static void ble_gattc_indicate_rx_rsp(struct ble_gattc_proc *proc)
{
    ...
    /* Tell the application about the received acknowledgment. */
    ble_gap_notify_tx_event(BLE_HS_EDONE, proc->conn_handle, proc->indicate.chr_val_handle, 1);
 ...
 */
void ble_gattc_rx_find_info_idata(uint16_t conn_handle, struct ble_att_find_info_idata *idata)
{
    ...
    proc = ble_gattc_extract_first_by_conn_op(conn_handle, BLE_GATT_OP_DISC_ALL_DSCS);
    if (proc != NULL) {
 ...
 */
void ble_gattc_rx_find_type_value_hinfo(uint16_t conn_handle, struct ble_att_find_type_value_hinfo *hinfo)
{
    ...
    proc = ble_gattc_extract_first_by_conn_op(conn_handle, BLE_GATT_OP_DISC_SVC_UUID);
    if (proc != NULL) {
 ...
 * to the appropriate active GATT procedure.
 */
void ble_gattc_rx_read_type_adata(uint16_t conn_handle, struct ble_att_read_type_adata *adata)
{
    ...
    proc = BLE_GATTC_RX_EXTRACT_RX_ENTRY(conn_handle, ble_gattc_rx_read_type_elem_entries, &rx_entry);
    if (proc != NULL) {
...
void ble_gattc_rx_read_type_complete(uint16_t conn_handle, int status)
{
    ...
    proc = BLE_GATTC_RX_EXTRACT_RX_ENTRY(conn_handle, ble_gattc_rx_read_type_complete_entries, &rx_entry);
    if (proc != NULL) {
 ...
 */
void ble_gattc_rx_read_group_type_adata(uint16_t conn_handle, struct ble_att_read_group_type_adata *adata)
{
    ...
    proc = ble_gattc_extract_first_by_conn_op(conn_handle, BLE_GATT_OP_DISC_ALL_SVCS);
    if (proc != NULL) {
 ...
 * read-by-group-type-response to the appropriate active GATT procedure.
 */
void ble_gattc_rx_read_group_type_complete(uint16_t conn_handle, int status)
{
#if !TY_HS_BLE_ATT_CLT_READ_GROUP_TYPE
    return;
#endif
 ...
 * procedure.
 */
void ble_gattc_rx_read_rsp(uint16_t conn_handle, int status, struct os_mbuf **om)
{
    ...
    proc = BLE_GATTC_RX_EXTRACT_RX_ENTRY(conn_handle, ble_gattc_rx_read_rsp_entries, &rx_entry);
    if (proc != NULL) {
 ...
 * procedure.
 */
void ble_gattc_rx_read_blob_rsp(uint16_t conn_handle, int status, struct os_mbuf **om)
{
    ...
    proc = ble_gattc_extract_first_by_conn_op(conn_handle, BLE_GATT_OP_READ_LONG);
    if (proc != NULL) {
 ...
 * GATT procedure.
 */
void ble_gattc_rx_read_mult_rsp(uint16_t conn_handle, int status, struct os_mbuf **om)
{
    ...
    proc = ble_gattc_extract_first_by_conn_op(conn_handle, BLE_GATT_OP_READ_MULT);
    if (proc != NULL) {
...
void ble_gattc_rx_write_rsp(uint16_t conn_handle)
{
    ...
    proc = ble_gattc_extract_first_by_conn_op(conn_handle, BLE_GATT_OP_WRITE);
    if (proc != NULL) {
 ...
 * GATT procedure.
 */
void ble_gattc_rx_prep_write_rsp(uint16_t conn_handle, int status, uint16_t handle, uint16_t offset,
                                 struct os_mbuf **om)
{
    ...
    proc = BLE_GATTC_RX_EXTRACT_RX_ENTRY(conn_handle, ble_gattc_rx_prep_entries, &rx_entry);
    if (proc != NULL) {
...
void ble_gattc_rx_exec_write_rsp(uint16_t conn_handle, int status)
{
    ...
    proc = BLE_GATTC_RX_EXTRACT_RX_ENTRY(conn_handle, ble_gattc_rx_exec_entries, &rx_entry);
    if (proc != NULL) {
        rc = rx_entry->cb(proc, status);
        ble_gattc_process_status(proc, rc);
    }
...
void ble_gattc_rx_indicate_rsp(uint16_t conn_handle)
{
    ...
    proc = ble_gattc_extract_first_by_conn_op(conn_handle, BLE_GATT_OP_INDICATE);
    if (proc != NULL) {
...
int ble_gattc_init(void)
{
    ...
    if ((TY_HS_BLE_GATT_MAX_PROCS) > 0) {
        rc = os_mempool_init(&ble_gattc_proc_pool, (TY_HS_BLE_GATT_MAX_PROCS), sizeof(struct ble_gattc_proc),
                             ble_gattc_proc_mem, "ble_gattc_proc_pool");
        if (rc != 0) {
            return rc;
        }
    }

    rc = stats_init_and_reg(STATS_HDR(ble_gattc_stats), STATS_SIZE_INIT_PARMS(ble_gattc_stats, STATS_SIZE_32),
